...
function refresh_options() {
	...
	for (var field in confs) {
		...
		if(c["type"] == "3" && c.v2ray_use_json == "1"){
			continue;
		}else if(c["type"] == "4" && c.v2ray_use_json == "1"){
			continue;
		}else{
...
function save() {
	...
	var params_check = ["ss_failover_enable", "ss_failover_c1", "ss_failover_c2", "ss_failover_c3", "ss_adv_sub", "ss_basic_tablet", "ss_basic_dragable", "ss_basic_qrcode", "ss_basic_enable", "ss_basic_gfwlist_update", "ss_basic_tnd", "ss_basic_vcore", "ss_basic_tcore", "ss_basic_xguard", "ss_basic_rust", "ss_basic_tjai", "ss_basic_olddns", "ss_basic_advdns", "ss_basic_chnroute_update", "ss_basic_cdn_update", "ss_basic_kcp_nocomp", "ss_basic_udp_boost_enable", "ss_basic_udpv1_disable_filter", "ss_basic_udpv2_disableobscure", "ss_basic_udpv2_disablechecksum", "ss_basic_udp2raw_boost_enable", "ss_basic_udp2raw_a", "ss_basic_udp2raw_keeprule", "ss_basic_dns_hijack", "ss_basic_chng_no_ipv6", "ss_basic_dohc_proxy", "ss_basic_dohc_ecs_china", "ss_basic_dohc_ecs_foreign", "ss_basic_dohc_cache_reuse", "ss_basic_chng_china_1_enable", "ss_basic_chng_china_2_enable", "ss_basic_chng_china_1_ecs", "ss_basic_chng_trust_1_enable", "ss_basic_chng_trust_2_enable", "ss_basic_chng_china_2_ecs", "ss_basic_chng_trust_1_ecs", "ss_basic_chng_trust_2_ecs" ];
	...
	if (db_ss["ssconf_basic_type_" + node_sel] =="0" ){
		...
		for (var i = 0; i < params_ssc_1.length; i++) {
			dbus["ssconf_basic_" + params_ssc_1[i] + "_" + node_sel] = E("ss_basic_" + params_ssc_1[i]).checked ? '1' : '';
		}
	}
	if (db_ss["ssconf_basic_type_" + node_sel] =="1" ){
		...
		var params_src_1 = ["use_kcp"];
		dbus["ssconf_basic_password_" + node_sel] = Base64.encode(E("ss_basic_password").value);
		for (var i = 0; i < params_sri_1.length; i++) {
			dbus["ssconf_basic_" + params_sri_1[i] + "_" + node_sel] = E("ss_basic_" + params_sri_1[i]).value;
		}
		for (var i = 0; i < params_src_1.length; i++) {
			if (E("ss_basic_" + params_src_1[i]).checked ? '1' : '0' != "0"){
				dbus["ssconf_basic_" + params_src_1[i] + "_" + node_sel] = E("ss_basic_" + params_src_1[i]).checked ? '1' : '';
			}
		}
	}
	if (db_ss["ssconf_basic_type_" + node_sel] =="3" ){
		if (E("ss_basic_v2ray_use_json").checked == true){
			...
			dbus["ssconf_basic_v2ray_use_json_" + node_sel] = "";
	...
	if (db_ss["ssconf_basic_type_" + node_sel] =="4" ){
		if (E("ss_basic_xray_use_json").checked == true){
			var params_xr_more = ["server", "port", "xray_uuid", "xray_encryption", "xray_flow", "xray_network", "xray_headtype_tcp", "xray_headtype_kcp", "xray_headtype_quic", "xray_grpc_mode", "xray_network_path", "xray_network_host", "xray_network_security", "xray_network_security_sni", "xray_network_security_ai", "xray_network_security_alpn_h2", "xray_network_security_alpn_http"];
			...
			}else if(E("ss_basic_xray_network_security").value == "xtls"){
...
function verifyFields(r) {
	...
	elem.display(elem.parentElem('ss_basic_xray_flow', 'tr'), (xray_on && x_json_off && x_xtls_on));
	...
	elem.display(elem.parentElem('ss_basic_xray_grpc_mode', 'tr'), (xray_on && x_json_off && E("ss_basic_xray_network").value == "grpc"));
	...
	elem.display(elem.parentElem('ss_basic_xray_network_security_sni', 'tr'), (xray_on && x_json_off && x_tls_on));
	...
	if (save_flag == "xray") {
		if(E("ss_node_table_xray_use_json").checked){
			...
			showhide("xray_headtype_tcp_tr", (E("ss_node_table_xray_network").value == "tcp"));
			...
			showhide("xray_grpc_mode_tr", (E("ss_node_table_xray_network").value == "grpc"));
			...
			showhide("xray_network_security_sni_tr", x_tls_on_2);
			showhide("xray_flow_tr", x_xtls_on_2);
		}
	}
	...
	if(E("ss_adv_sub").checked == false){
		...
		$("#ssr_subscribe_obfspara").parent().parent().hide();
		...
		$("#ssr_subscribe_obfspara").parent().parent().show();
...
function update_visibility() {
	...
	var j3 = E("ss_basic_chng_china_1_doh").value == "98";
	...
	var k3 = E("ss_basic_chng_china_2_doh").value == "98";
	...
	var m3 = E("ss_basic_chng_trust_2_opt_doh").value;
	var n = E("ss_basic_smrt").value;
	var n1 = E("ss_basic_smrt").value == "1";
	var n2 = E("ss_basic_smrt").value == "2";
	var n3 = E("ss_basic_smrt").value == "3";
	var o = E("ss_basic_dohc_sel_china").value;
	var p1 = E("ss_basic_dohc_udp_china").value == "99";
	var p2 = E("ss_basic_dohc_tcp_china").value == "99";
	var q = E("ss_basic_dohc_sel_foreign").value;
	var r = E("ss_basic_dohc_tcp_foreign").value == "99";
	...
	showhide("ss_basic_s_resolver_doh", (h == "3"));
	...
	showhide("ss_basic_chng_china_1_doh", (j && j0 == "3"));
	showhide("dohclient_cache_manage_chn1", (j && j0 == "3" && !j3));
	showhide("edit_smartdns_conf_10", (j && j0 == "1" && j1));			//udp smartdns
	showhide("edit_smartdns_conf_11", (j && j0 == "2" && j2));			//tcp smartdns
	showhide("edit_smartdns_conf_12", (j && j0 == "3" && j3));			//doh smartdns
	if (j == true){
		if(j0 == "1" && j1){
	...
	showhide("ss_basic_chng_china_2_doh", (k && k0 == "3"));
	showhide("dohclient_cache_manage_chn2", (k && k0 == "3" && !k3));
	showhide("edit_smartdns_conf_13", (k && k0 == "1" && k1));
	showhide("edit_smartdns_conf_14", (k && k0 == "2" && k2));
	showhide("edit_smartdns_conf_15", (k && k0 == "3" && k3));
	if (k == true){
		if(k0 == "1" && k1){
	...
	showhide("ss_basic_chng_trust_1_opt_doh_val", (l && l0 == "3"));
	showhide("dohclient_cache_manage_frn1", (l && l0 == "3"));
	...
	showhide("ss_basic_chng_trust_2_opt_doh", (m && m0 == "3"));
	showhide("edit_smartdns_conf_30", (m && m0 == "3" && m3 == "97"));
	showhide("dohclient_cache_manage_frn2", (m && m0 == "3" && m3 != "97"));
	if (m == true){
		if( m0 == "3" && m3 == "97" ){
	...
	showhide("edit_smartdns_conf_40", (h == "2"));
	showhide("edit_smartdns_conf_51", (n == "1"));
	showhide("edit_smartdns_conf_52", (n == "2"));
	showhide("edit_smartdns_conf_53", (n == "3"));
	showhide("edit_smartdns_conf_54", (n == "4"));
	showhide("edit_smartdns_conf_55", (n == "5"));
	showhide("edit_smartdns_conf_56", (n == "6"));
	showhide("edit_smartdns_conf_57", (n == "7"));
	showhide("edit_smartdns_conf_58", (n == "8"));
	showhide("edit_smartdns_conf_59", (n == "9"));
	showhide("ss_basic_dohc_udp_china", (o == "1"));
	showhide("ss_basic_dohc_tcp_china", (o == "2"));
	showhide("ss_basic_dohc_doh_china", (o == "3"));
	showhide("ss_basic_dohc_udp_china_user", (o == "1" && p1));
	showhide("ss_basic_dohc_tcp_china_user", (o == "2" && p2));
	showhide("ss_basic_dohc_tcp_foreign", (q == "2"));
	showhide("ss_basic_dohc_doh_foreign", (q == "3"));
	showhide("ss_basic_dohc_tcp_foreign_user", (q == "2" && r));
...
function add_ss_node_conf(flag) {
	...
	if (flag == 'shadowsocks') {
		...
		var params5_1 = ["mode", "name", "server", "port", "xray_uuid", "xray_encryption", "xray_flow", "xray_network", "xray_headtype_tcp", "xray_headtype_kcp", "xray_headtype_quic", "xray_grpc_mode", "xray_network_path", "xray_network_host", "xray_network_security", "xray_network_security_sni"]; //for xray
		var params5_2 = ["xray_use_json", "xray_network_security_ai", "xray_network_security_alpn_h2", "xray_network_security_alpn_http"];
...
function refresh_html() {
	...
	for (var i = 0; i < ss_nodes.length; i++) {
		...
		if(c["type"] == 6){
			...
			html += '<div class="server">' + c["naive_server"] + '</div>';
			html += '</td>';
		}else{
...
function toggle_func() {
	...
	$(".show-btn3").click(
		function() {
			...
			change_select_width('#ss_basic_chng_china_1_doh', '1');
			change_select_width('#ss_basic_chng_china_2_udp', '1');
			change_select_width('#ss_basic_chng_china_2_tcp', '1');
			change_select_width('#ss_basic_chng_china_2_doh', '1');
			change_select_width('#ss_basic_chng_trust_1_opt_udp_val', '1');
			change_select_width('#ss_basic_chng_trust_1_opt_tcp_val', '1');
			change_select_width('#ss_basic_chng_trust_1_opt_doh_val', '1');
			change_select_width('#ss_basic_chng_trust_2_opt_doh', '1');
			change_select_width('#ss_basic_smrt');
			change_select_width('#ss_basic_dohc_udp_china', '1');
			change_select_width('#ss_basic_dohc_tcp_china', '1');
			change_select_width('#ss_basic_dohc_doh_china', '1');
			change_select_width('#ss_basic_dohc_tcp_foreign', '1');
			change_select_width('#ss_basic_dohc_doh_foreign', '1');
			change_select_width('#ss_basic_dohc_cache_timeout', '0');
			...
			change_select_width('#ss_basic_s_resolver_doh');
														...
														<table id="table_edit" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" class="FormTable">
															<script type="text/javascript">
																$('#table_edit').forms([
																	...
																	{ title: 'v2ray-plugin', rid:'ss_v2ray_support', id:'ss_node_table_ss_v2ray', type:'select', func:'v', options:[["0", "关闭"], ["1", "开启"]], style:'width:412px', value: "0"},
																	{ title: 'v2ray-plugin参数', rid:'ss_v2ray_opts_support', id:'ss_node_table_ss_v2ray_opts', type:'text', maxlen:'300', style:'width:400px', ph:'tls;host=example.com;path=/'},
											...
											<table id="table_basic" width="100%" border="0" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable">
												<script type="text/javascript">
													$('#table_basic').forms([
														...
														{ title: 'flow (流控模式，用于选择 XTLS 的算法)', id:'ss_basic_xray_flow', type:'select', hint:'56', options:option_xflow},
														...
														{ title: '* 路径 (path)', id:'ss_basic_xray_network_path', type:'text', hint:'29', maxlen:'300', ph:'没有请留空'},
														{ title: '* kcp seed', id:'ss_basic_xray_kcp_seed', type:'text', maxlen:'300', ph:'没有请留空'},
														{ title: '底层传输安全', id:'ss_basic_xray_network_security', type:'select', func:'v', hint:'30', options:[["none", "关闭"], ["tls", "tls"], ["xtls", "xtls"]]},
														...
														{ title: '* SNI', id:'ss_basic_xray_network_security_sni', type:'text'},
										...
										<div id="tablet_3" style="display: none;">
											<table id="table_dns" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable">
												<script type="text/javascript">
													...
													option_dnsngc_udp.push(["group", "SmartDNS"]);
													option_dnsngc_udp.push(["96", "⚫SmartDNS (UDP)"]);
													...
													var option_dnsngf_2_val = [
																		 	   ["group", "国外直连组 (dohclient)"],
																	  		   ["11", "⚫Cloudflare"],
																	  		   ["16", "⚫OpenDNS"],
																	  		   ["17", "⚫DNS.SB"],
																	   		   ["18", "⚫cleanbrowsing"],
													...
													var option_smrt = [
																	   ["1", "1：国内：运营商DNS；国外：代理"],
																	   ["2", "2：国内：运营商DNS，国外：直连"],
																	   ["3", "3：国内：运营商DNS，国外：代理 + 直连"],
																	   ["4", "4：国内：多上游DNS，国外：代理"],
																	   ["5", "5：国内：多上游DNS，国外：直连"],
																	   ["6", "6：国内：多上游DNS，国外：代理 + 直连"],
																	   ["7", "7：自定义配置1"],
																	   ["8", "8：自定义配置2"],
																	   ["9", "9：自定义配置3"]
																	  ];
													option_protc_sel_chn = [
																		  ["1", "udp"],
																		  ["2", "tcp"],
																		  ["3", "DoH"]
																		 ];
													option_protc_sel_frn = [
																		  ["2", "tcp"],
																		  ["3", "DoH"]
																		 ];
													var option_dohc_udp_china = [];
													if(isp_dns_1 && isp_dns_2){
														option_dohc_udp_china.push(["group", "运营商DNS"]);
														option_dohc_udp_china.push(["1", "⚪" + isp_dns_1]);
														option_dohc_udp_china.push(["2", "⚪" + isp_dns_2]);
													}else if(isp_dns_1 && !isp_dns_2){
														option_dohc_udp_china.push(["group", "运营商DNS"]);
														option_dohc_udp_china.push(["1", "⚪" + isp_dns_1]);
													}
													option_dohc_udp_china.push(["group", "阿里公共DNS"]);
													option_dohc_udp_china.push(["3", "🟠223.5.5.5"]);
													option_dohc_udp_china.push(["4", "🟠223.6.6.6"]);
													option_dohc_udp_china.push(["group", "DNSPod DNS"]);
													option_dohc_udp_china.push(["5", "🟠119.29.29.29"]);
													option_dohc_udp_china.push(["6", "🟠119.28.28.28"]);
													option_dohc_udp_china.push(["group", "114 DNS"]);
													option_dohc_udp_china.push(["7", "⚫114.114.114.114"]);
													option_dohc_udp_china.push(["8", "⚫114.114.115.115"]);
													option_dohc_udp_china.push(["group", "OneDNS"]);
													option_dohc_udp_china.push(["9", "🟠117.50.11.11（拦截版）"]);
													option_dohc_udp_china.push(["10", "🟠52.80.66.66（拦截版）"]);
													option_dohc_udp_china.push(["11", "🟠117.50.10.10（纯净版）"]);
													option_dohc_udp_china.push(["12", "🟠52.80.52.52（纯净版）"]);
													option_dohc_udp_china.push(["13", "🟠117.50.60.30（家庭版）"]);
													option_dohc_udp_china.push(["14", "🟠52.80.60.30（家庭版）"]);
													option_dohc_udp_china.push(["group", "360安全DNS"]);
													option_dohc_udp_china.push(["15", "🟠101.226.4.6（电信/铁通/移动）"]);
													option_dohc_udp_china.push(["16", "🟠218.30.118.6（电信/铁通/移动）"]);
													option_dohc_udp_china.push(["17", "🟠123.125.81.6（联通）"]);
													option_dohc_udp_china.push(["18", "🟠140.207.198.6（联通）"]);
													option_dohc_udp_china.push(["group", "cnnic DNS"]);
													option_dohc_udp_china.push(["19", "⚫1.2.4.8"]);
													option_dohc_udp_china.push(["20", "⚫210.2.4.8"]);
													option_dohc_udp_china.push(["group", "百度DNS"]);
													option_dohc_udp_china.push(["21", "🟠180.76.76.76"]);
													option_dohc_udp_china.push(["group", "教育网DNS"]);
													option_dohc_udp_china.push(["22", "🟠101.6.6.6:5353（清华大学）"]);
													option_dohc_udp_china.push(["23", "⚫58.132.8.1（北京）"]);
													option_dohc_udp_china.push(["24", "⚫101.7.8.9（北京）"]);
													option_dohc_udp_china.push(["group", "自定义DNS"]);
													option_dohc_udp_china.push(["99", "⚪自定义DNS (udp)"]);
													option_dohc_tcp_china = [
																			 ["group", "阿里公共DNS"],
																			 ["3", "🟠223.5.5.5"],
																			 ["4", "🟠223.6.6.6"],
																			 ["group", "DNSPod DNS"],
													...
													option_dohc_doh_china = [
																			 ["1", "🟠阿里公共DNS"],
																			 ["2", "🟠DNSPod公共DNS"],
																			 ["3", "🟠360安全DNS"]
																			];
													var option_dohc_tcp_foreign = [
																	   ...
																	   ["7", "OpenDNS [208.67.220.220]"],
																	   ["8", "DNS.SB [185.222.222.222]"],
																	   ["9", "DNS.SB [45.11.45.11]"],
																	   ["10", "quad101 [101.101.101.101]"],
																	   ["11", "quad101 [101.102.103.104]"],
																	   ["12", "AdGuard [94.140.14.14]"],
																	   ["13", "AdGuard [94.140.15.15]"],
																	   ["99", "自定义DNS(tcp)"]
																	  ];
													var option_dohcf = [
																	   ["11", "⚫Cloudflare"],
													...
													var option_cache_timeout = [["0", "从不过期"], ["1", "根据ttl"], ["1800", "半小时"], ["3600", "一小时"], ["7200", "两小时"], ["43200", "12小时"], ["86400", "24小时"]];
													var option_chndns = [];
													if(isp_dns_1 && isp_dns_2){
													...
													option_chndns.push(["group", "SmartDNS"]);
													option_chndns.push(["98", "SmartDNS (UDP)"]);
													...
													$('#table_dns').forms([
														...
														{ title: '&nbsp;&nbsp;*选择中国DNS-1 <em>(直连) 🌏</em>', hint:'133', class:'new_dns chng', multi: [
															...
															{ id: 'ss_basic_chng_china_1_doh', type:'select', func:'u', options:option_dnsngc_doh, style:'width:200px;', value:'1'},
															{ suffix:'&nbsp;&nbsp;'},
															{ suffix:'<a type="button" id="edit_smartdns_conf_10" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(10)">编辑smartdns配置</a>'},
															{ suffix:'<a type="button" id="edit_smartdns_conf_11" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(11)">编辑smartdns配置</a>'},
															{ suffix:'<a type="button" id="edit_smartdns_conf_12" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(12)">编辑smartdns配置</a>'},
															...
															{ suffix: '<a type="button" id="dohclient_cache_manage_chn1" class="ss_btn" style="cursor:pointer" target="_blank" href="http://' + '<% nvram_get("lan_ipaddr"); %>' + ':2051">缓存管理</a>' },
														]},
														{ title: '&nbsp;&nbsp;*选择中国DNS-2 <em>(直连) 🌏</em>', hint:'133', class:'new_dns chng', multi: [
															...
															{ id: 'ss_basic_chng_china_2_doh', type:'select', func:'u', options:option_dnsngc_doh, style:'width:200px;', value:'1'},
															{ suffix:'&nbsp;&nbsp;'},
															{ suffix:'<a type="button" id="edit_smartdns_conf_13" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(10)">编辑smartdns配置</a>'},
															{ suffix:'<a type="button" id="edit_smartdns_conf_14" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(11)">编辑smartdns配置</a>'},
															{ suffix:'<a type="button" id="edit_smartdns_conf_15" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(12)">编辑smartdns配置</a>'},
															...
															{ suffix: '<a type="button" id="dohclient_cache_manage_chn2" class="ss_btn" style="cursor:pointer" target="_blank" href="http://' + '<% nvram_get("lan_ipaddr"); %>' + ':2052">缓存管理</a>' },
														]},
														{ title: '&nbsp;&nbsp;*选择可信DNS-1 <font color="#FF0066">(代理) 🚀</font>', hint:'134', class:'new_dns chng', rid:'dns_plan_foreign_1', multi: [
															...
															{ id: 'ss_basic_chng_trust_1_opt_doh_val', type:'select', func:'u', options:option_dnsngf_1_val_doh, style:'width:auto;', value:'12'},
															{ suffix: '&nbsp;&nbsp;'},
															...
															{ suffix: '<a type="button" id="dohclient_cache_manage_frn1" class="ss_btn" style="cursor:pointer" target="_blank" href="http://' + '<% nvram_get("lan_ipaddr"); %>' + ':2055">缓存管理</a>' },
														]},
														{ title: '&nbsp;&nbsp;*选择可信DNS-2 <em>(直连) 🌏</em>', class:'new_dns chng', hint:'135', rid:'dns_plan_foreign_2', multi: [
															...
															{ id: 'ss_basic_chng_trust_2_opt_doh', type:'select', func:'u', options:option_dnsngf_2_val, style:'width:auto;', value:'2'},
															{ suffix: '&nbsp;&nbsp;'},
															{ suffix: '<a type="button" id="edit_smartdns_conf_30" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(30)">编辑smartdns配置</a>'},
															...
															{ suffix: '<a type="button" id="dohclient_cache_manage_frn2" class="ss_btn" style="cursor:pointer" target="_blank" href="http://' + '<% nvram_get("lan_ipaddr"); %>' + ':2056">缓存管理</a>' },
														]},	
														...
														{ title: '&nbsp;&nbsp;*选择smartdns配置', class:'new_dns smrt', multi: [
															{ id: 'ss_basic_smrt', type:'select', func:'u', options:option_smrt, style:'width:260px;', value:'1'},
															{ suffix: '&nbsp;&nbsp;'},
															{ suffix: '<a type="button" id="edit_smartdns_conf_51" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(51)">编辑smartdns配置</a>'},
															{ suffix: '<a type="button" id="edit_smartdns_conf_52" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(52)">编辑smartdns配置</a>'},
															{ suffix: '<a type="button" id="edit_smartdns_conf_53" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(53)">编辑smartdns配置</a>'},
															{ suffix: '<a type="button" id="edit_smartdns_conf_54" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(54)">编辑smartdns配置</a>'},
															{ suffix: '<a type="button" id="edit_smartdns_conf_55" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(55)">编辑smartdns配置</a>'},
															{ suffix: '<a type="button" id="edit_smartdns_conf_56" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(56)">编辑smartdns配置</a>'},
															{ suffix: '<a type="button" id="edit_smartdns_conf_57" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(57)">编辑smartdns配置</a>'},
															{ suffix: '<a type="button" id="edit_smartdns_conf_58" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(58)">编辑smartdns配置</a>'},
															{ suffix: '<a type="button" id="edit_smartdns_conf_59" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(59)">编辑smartdns配置</a>'},
														]},
														{ title: '&nbsp;&nbsp;*选择国内DNS', class:'new_dns dohc', hint:'122', multi: [
															{ id: 'ss_basic_dohc_sel_china', type:'select', func:'u', options:option_protc_sel_chn, style:'width:50px;', value:'3'},
															{ id: 'ss_basic_dohc_udp_china', type:'select', func:'u', options:option_dnsngc_udp, style:'width:190px;', value:'3'},
															{ id: 'ss_basic_dohc_udp_china_user', type: 'text', style:'width:110px;', ph:'114.114.114.114' },
															{ id: 'ss_basic_dohc_tcp_china', type:'select', func:'u', options:option_dohc_tcp_china, style:'width:190px;', value:'1'},
															{ id: 'ss_basic_dohc_tcp_china_user', type: 'text', style:'width:110px;', ph:'114.114.114.114' },
															{ id: 'ss_basic_dohc_doh_china', type:'select', func:'u', options:option_dohc_doh_china, style:'width:190px;', value:'1'},
															{ prefix: '&nbsp;<a class="hintstyle" href="javascript:void(0);" onclick="openssHint(123)"><font color="#ffcc00"><u>ECS</u></font></a>', id: 'ss_basic_dohc_ecs_china', type: 'checkbox', value:true },
														]},
														{ title: '&nbsp;&nbsp;*选择国外DNS', class:'new_dns dohc', hint:'124', multi: [
															{ id: 'ss_basic_dohc_sel_foreign', type:'select', func:'u', options:option_protc_sel_frn, style:'width:50px;', value:'3'},
															{ id: 'ss_basic_dohc_tcp_foreign', type:'select', func:'u', options:option_dohc_tcp_foreign, style:'width:190px;', value:'3'},
															{ id: 'ss_basic_dohc_tcp_foreign_user', type: 'text', style:'width:110px;', ph:'8.8.8.8' },
															{ id: 'ss_basic_dohc_doh_foreign', type:'select', func:'u', options:option_dohcf, style:'width:190px;', value:'12'},
															{ prefix: '&nbsp;<a class="hintstyle" href="javascript:void(0);" onclick="openssHint(125)"><font color="#ffcc00"><u>ECS</u></font></a>', id: 'ss_basic_dohc_ecs_foreign', type: 'checkbox', value:true },
															{ prefix: '&nbsp;<a class="hintstyle" href="javascript:void(0);" onclick="openssHint(126)"><font color="#ffcc00"><u>Proxy</u></font></a>', id: 'ss_basic_dohc_proxy', type: 'checkbox', value:true },
														]},
														{ title: '&nbsp;&nbsp;*缓存管理', class:'new_dns dohc', multi: [
															{ prefix: '&nbsp;&nbsp;<a class="hintstyle" href="javascript:void(0);" onclick="openssHint(127)"><font color="#ffcc00"><u>缓存时长</u>：</font></a>' },
															{ id: 'ss_basic_dohc_cache_timeout', type:'select', func:'u', options:option_cache_timeout, style:'width:100px;', value:'1'},
															{ suffix: '&nbsp;&nbsp;<a type="button" id="dohclient_cache_manage_dohc" class="ss_btn" style="cursor:pointer" target="_blank" href="http://' + '<% nvram_get("lan_ipaddr"); %>' + ':7913">缓存管理</a>' },
															{ suffix: '&nbsp;&nbsp;<a type="button" class="ss_btn" style="cursor:pointer" onclick="remove_doh_cache(1)">清空缓存</a>&nbsp;&nbsp;'},
															{ prefix: '&nbsp;&nbsp;<a class="hintstyle" href="javascript:void(0);" onclick="openssHint(128)"><font color="#ffcc00"><u>缓存持久化</u></font></a>', id: 'ss_basic_dohc_cache_reuse', type: 'checkbox', value:false },
														]},
														...
														{ title: '节点域名解析DNS方案', hint:'107', multi: [
															...
															{ id: 'ss_basic_s_resolver_doh', type:'select', func:'u', options:option_dohc_doh_resv, style:'width:160px;', value:'3'},
															{ suffix: '&nbsp;&nbsp;' },
															{ suffix: '<a type="button" id="edit_smartdns_conf_40" class="ss_btn" style="cursor:pointer" onclick="edit_smartdns_conf(40)">编辑smartdns配置</a>'},
														]},
														{ title: '自定义dnsmasq', id:'ss_dnsmasq', type:'textarea', hint:'34', rows:'12', ph:ph3},
													]);
													var curr_host = window.location.hostname;
													$("#dohclient_cache_manage_chn1").attr("href", "http://" + curr_host + ":2051");
													$("#dohclient_cache_manage_chn2").attr("href", "http://" + curr_host + ":2052");
													$("#dohclient_cache_manage_frn1").attr("href", "http://" + curr_host + ":2055");
													$("#dohclient_cache_manage_frn2").attr("href", "http://" + curr_host + ":2056");
													$("#dohclient_cache_manage_dohc").attr("href", "http://" + curr_host + ":7913");
												</script>
											</table>
										</div>
												...
												<script type="text/javascript">
													...
													$('#table_addons').forms([
														...
														{ title: '&nbsp;&nbsp;&nbsp;&nbsp;用shadowsocks-rust替代shadowsocks-libev', hint:'118', multi: [
															...
															{ suffix: '&nbsp;&nbsp;<a type="button" class="ss_btn" style="cursor:pointer" onclick="ssrust_binary_update(2)">下载 shadowsocks-rust 二进制</a>'}
														]},
